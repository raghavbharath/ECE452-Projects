...
% We added this portion down below: 
% This basically scales inputs to [-1,1] using saved mapminmax parameters to match toolbox preprocessing.
gain   = export.preprocess.gain;
offset = export.preprocess.offset;
Xte_scaled = bsxfun(@times, gain, bsxfun(@minus, Xte, offset)) + (-1);

% ====================== TODO 1: CUSTOM FORWARD PASS ======================
% Implement forward_mlp(X, W, b, hiddenAct) and use it here:

Ypred = forward_mlp(Xte_scaled, export.W, export.b, export.netInfo.hiddenAct);
% =========================================================================
...
Y_debug = forward_mlp(Xte(:,1:5), export.W, export.b, export.netInfo.hiddenAct);
[~, pred_debug] = max(Y_debug, [], 1);
[~, true_debug] = max(Yte(:,1:5), [], 1);
disp(pred_debug)
disp(true_debug)
...
function A = activation(Z, hiddenAct)
    if hiddenAct == "logsig"
        A = 1 ./ (1 + exp(-Z));
    elseif hiddenAct == "tansig"
        A = 2 ./ (1 + exp(-2*Z)) - 1;
    elseif hiddenAct == "poslin"
        A = max(0, Z);
    end
end
